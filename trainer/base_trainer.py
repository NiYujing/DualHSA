...
class BaseTrainer:
    ...
    def __init__(self, model, criterion, metric_fns, optimizer, config):
        ...
        self.best_epoch = None  # 添加一个属性来存储最佳性能的epoch编号
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            # 对每个epoch，调用 _train_epoch 方法进行单个 epoch 的训练
            result = self._train_epoch(epoch)

            # save logged informations into log dict --- log是日志字典
            log = {'epoch': epoch}
            log.update(result)
            ...
            if self.mnt_mode != 'off':
                ...
                # 连续 early_stop 个 epoch 模型性能都没有改善
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        if save_best:
            ...
            # best_path = str(self.checkpoint_dir / f'model_best_epoch{epoch}.pth')
            best_path = str(self.checkpoint_dir / 'model_best.pth')
            torch.save(state, best_path)
            self.logger.info(f"Saving current best: {epoch} ...")
        else:
